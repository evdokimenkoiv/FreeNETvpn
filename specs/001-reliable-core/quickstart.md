...
Select this feature using `python tools/spec_context.py 001-reliable-core`; then `python .specify/scripts/python/check_prerequisites.py --json --require-spec --require-tasks --include-tasks` must resolve its spec, plan, tasks and supporting documents. Run `python tools/spec_audit.py` to validate cross-artifact references. Agent workflow and platform setup: docs/spec-kit.md.
